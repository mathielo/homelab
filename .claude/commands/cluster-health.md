...
- **Disk** — one row per mount (`Status | Mount | Size | Used | Use%`) for `/`,
  ...
  ⚪ regardless of Use%; never 🟡/🔴 and never carry it to the §7 warnings sweep.**
...
`/var/lib/k3s/agent`); note ≥85% but know kubelet image-GC self-prunes around
85% of the image filesystem.
